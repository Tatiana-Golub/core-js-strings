...
function removeFirstOccurrences(str, value) {
  ...
  const substrToRemove = value;
  const index = originalStr.indexOf(substrToRemove);
  if (index !== -1) {
    const newStr =
      originalStr.substring(0, index) +
      originalStr.substring(index + substrToRemove.length);
    return newStr;
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const originalStr = str;
  const substrToRemove = value;
  const index = originalStr.lastIndexOf(substrToRemove);
  if (index !== -1) {
    const newStr =
      originalStr.substring(0, index) +
      originalStr.substring(index + substrToRemove.length);
...
function startsWith(str, substr) {
  const text = str;
  const result = text.startsWith(substr);
  return result;
}
...
function endsWith(str, substr) {
  const text = str;
  const result = text.endsWith(substr);
  return result;
}
...
function getCardId(value) {
  ...
  const search = arr.indexOf(value);
  return search;
}
